...
- (id)initWithWidth:(CGFloat)width Height:(CGFloat)heigth
{
    self = [super init];
...
- (void)useDefaultMetricsAndInvalidate:(BOOL)invalidate
{
    ...
    //m.normal.size       = CGSizeMake(320.0, 200.0);//420
    m.normal.size       = CGSizeMake(_itemWidth, _itemHeight);//420
    m.normal.overlap    = 0.0;
    //m.collapsed.size    = CGSizeMake(320.0, 96.0);//96
    m.collapsed.size    = CGSizeMake(_itemWidth, 96.0);//96
    m.collapsed.overlap = 32.0;//32
